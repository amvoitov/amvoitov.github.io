...
case_title_short: "Весовая бункера-перегрузчика для контроля убраного зерна"
...
4. **Эксплуатация и поддержка, доработка нового функционала:**  ноября 2022 - февраль 2025
...
### Входило в scope
- Учёт операций БП с **корректной идентификацией** техники/событий и сохранением результата в системе владельца.
- Android-приложение (планшет): отображение веса, логика/фильтрация, **оффлайн-накопление** и последующая синхронизация.
- Передача данных на сервер: отправка при наличии сети, при отсутствии — **пакетная выгрузка** после восстановления.
- Веб-кабинет/портал: хранение и отображение данных операций/чеков/гео/времени/идентификаторов техники (владельцу/ролям).
- Ключевые требования: **точность ±1%**, диапазон до **100 т**, условия **вибрации/удары**, обновление на экране **≥10 Гц**.
- Канал связи “блок → Android”: **Bluetooth**, передача **«сырых значений»**.

### Вне scope (если потребуется рынком — расширение)
- Расширенные сценарии инженерки/ошибок/оборудования (вынесено в развитие после базовой версии).
- Дополнительные “нестандартные” интеграции партнёров сверх базового набора (по запросу/пилотам).

## Архитектура данных (поток в 1 абзаце)

Сигнал веса формируется на технике и обрабатывается в блоке, далее по **Bluetooth** на Android передаются **сырые значения**, где выполняется отображение/логика/фильтрация и накопление операций в **offline-очереди** при отсутствии интернета. При наличии связи Android отправляет на сервер операции пакетами/по событиям. В backend данные сохраняются с правилами доступа и становятся доступны владельцу в **web-кабинете** (включая чек/время/гео/идентификаторы техники).  

## Критерии готовности (Definition of Done)

Система считается “готовой к внедрению”, если выполняются условия:

1. **Точность**: обеспечена погрешность **±1%** на диапазоне до **100 т** в условиях вибраций/ударов (по процедуре испытаний/валидации).
2. **Частота обновления**: Android отображает и обновляет данные **не ниже 10 Гц**.
3. **Оффлайн-режим**: операции корректно сохраняются в памяти устройства при отсутствии интернета и не “теряются” при перезапуске приложения/планшета (в пределах доступной памяти).
4. **Синхронизация**: при восстановлении связи накопленные операции выгружаются на сервер **пакетами** без потерь/дубликатов (контроль логики отправки).
5. **Полнота данных операции**: операция в системе содержит минимум: тип (загрузка/выгрузка), время, GPS, идентификацию техники, плюс **чек** (сформирован/сохранён).
6. **Надёжность доставки**: протокол устойчив к разрывам: используются **ack/ретраи/переподключение**, а доля повторов/разрывов контролируется метриками.
7. **Доступность для владельца**: данные операций за период видны владельцу в web-кабинете согласно правилам доступа; “подтверждённый учтённый вес” корректно агрегируется.
